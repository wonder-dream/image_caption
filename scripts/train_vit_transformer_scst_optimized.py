"""
ViT + Transformer 图像描述模型 - SCST强化学习 + 优化技术 训练脚本
...
class RLMetricsHistory:
    ...
    def plot_and_save(self, save_path):
        ...
        fig.suptitle("SCST + Optimizations Training Metrics", fontsize=16)
...
def train_scst_optimized(config):
    ...
    if config.get("use_tensorboard", False):
        writer = SummaryWriter(log_dir=config.get("log_dir", "runs/vit_transformer_scst_opt"))
...
if __name__ == "__main__":
    config = {
        ...
        # 预训练模型路径 - 使用绝对路径
        "pretrain_checkpoint": os.path.join(PROJECT_ROOT, "checkpoints", "vit_transformer_optimized", "best_model.pth"),
        ...
        # 评估和保存 - 使用绝对路径
        "eval_every": 1,
        "checkpoint_dir": os.path.join(PROJECT_ROOT, "checkpoints", "vit_transformer_scst_opt"),
        
        # 日志 - 使用绝对路径
        "use_tensorboard": True,
        "log_dir": os.path.join(PROJECT_ROOT, "runs", "vit_transformer_scst_opt"),
    }

    print("=" * 70)
    print("ViT + Transformer 图像描述模型 - SCST + 优化技术 训练")
    print("=" * 70)
    print("\n配置:")
